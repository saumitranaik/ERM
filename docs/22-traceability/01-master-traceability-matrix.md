...
| Governed KPI/metric catalogue, threshold/banding measurement, and dashboard visualization composition (heat maps, trend lines, drill-downs) — the analytical layer `14-reporting` explicitly deferred, closing the "Reporting and Analytics" Supporting Subdomain `04-domain-model` reserved | None newly cited — operationalizes the recurring risk-appetite/KRI-monitoring (`10-risk`), Non-Compliance-Rate/Rectification-Index (`13-audit`, Annexures §1.3.4.1.1(ii)(d)–(f)), SLA-benchmarking (`25-third-party-risk`, §2.9.3.1(v)(c)), control-effectiveness (`12-controls`, §2.11.2.1(iii)), and RTO/RPO-testing (`26-business-continuity`, Annexure 8 item 8d) obligations already carried | [`15-analytics/01-analytics-management.md`](../15-analytics/01-analytics-management.md) |
...
| Analytics Management (`ANALYTICS` module) | RBAC + module role model (`§8`); module framework and ownership guards (`§9, §5a–§5c`, OWN-03/04/07/08/09); multi-tenant RLS (`§7`); observability trace contract (`§4.1`); audit trail (`§10`); authentication (`§21`). Governance ledger **not exercised** — this module defines zero `module_actions`-governed types (spec Assumption 8), the smallest governance footprint of any module in this repository. | None newly introduced beyond the already-flagged scheduled-job/batch-execution gap (re-affirmed, not re-discovered, from `14-reporting` Assumption 8). **One new gap named for the first time**: a generic formula-execution/BI-computation engine (spec Assumption 7), confirmed absent this session, not blocking MVP scope (formulas remain human-readable, computed in application code). **Identical dependency membership to `REPORTING`** (`dependencies: [RISK, CONTROLS, COMPLIANCE, AUDIT, SECURITY, POLICY, INCIDENT, TPR, BCP]`), but not a pure sink like `REPORTING`/`AUDIT` — `REPORTING` itself becomes a consumer of this module (proposed, not applied, per spec Assumption 11) to resolve its own already-reserved `DashboardWidget.widget_type = METRIC_REFERENCE` slot with **zero** additive change to `14-reporting`'s schema. **Proposes, does not apply**: an eleventh-bounded-context amendment to `04-domain-model` (spec Assumption 3) and a `dependencies:`/`DashboardDefinition.audience = REGULATORY` addition to `14-reporting` (spec Assumption 11) — following the established propose-then-apply-later pattern. No frozen spec was modified this session. |
...
| KPI/metric catalog, threshold/banding measurement, and dashboard visualization composition (heat maps, trend lines, drill-downs) (SEBI AMC profile) | [`15-analytics/01-analytics-management.md`](../15-analytics/01-analytics-management.md) | **Closed (2026-07-22, Session 16)** — activates the KPI/metric-catalog half of the "Reporting and Analytics" Supporting Subdomain `14-reporting/01-*` Assumption 16 explicitly deferred; a 48-row seed catalogue (42 consolidated, 6 new cross-module composite metrics); activates `14-reporting`'s already-reserved `DashboardWidget.widget_type = METRIC_REFERENCE` slot with **zero** additive change to that module's schema. |
| Generic formula-execution/BI-computation engine | No owning section identified yet | **Gap** — named for the first time by `15-analytics/01-*` (spec Assumption 7); confirmed absent this session; `MetricDefinition.formula_description` remains human-readable text, not an executable expression. |
| `04-domain-model` eleventh-bounded-context amendment: add `ANALYTICS` (Supporting Subdomain, alongside `REPORTING`) to the Bounded Context Map, Strategic Classification, Ownership Responsibilities, Canonical Business Glossary, Cross-Context APIs table, and Dependency Rule 5 | `04-domain-model/01-enterprise-domain-model.md` | **Proposed, not applied (2026-07-22, Session 16)** — proposed by `15-analytics/01-*` Assumption 3; follows the established propose-then-apply-later pattern. |
| `14-reporting` amendment: `dependencies:` gains `ANALYTICS`; `DashboardDefinition.audience` gains `REGULATORY` | `14-reporting/01-reporting-management.md` | **Proposed, not applied (2026-07-22, Session 16)** — proposed by `15-analytics/01-*` Assumption 11; activates the resolution direction of `GET /metric-views/{id}/reference` with zero schema change to `14-reporting`'s existing `metric_ref_id` column. |
| Roster-of-required-acknowledgers capability (needed to compute a true Policy Acknowledgement Completion *rate*, not merely a numerator) | No owning section identified yet | **Gap** — named for the first time by `15-analytics/01-*` (`MTR-POL-001`); `23-policy/01-*` itself already states it records who *did* acknowledge, not who *must*. |
...
**Session 16 (2026-07-22)** authored the repository's **twelfth authoritative specification**,
...
visualization composition layer `14-reporting/01-*` Assumption 16 explicitly deferred, closing
the "Reporting and Analytics" Supporting Subdomain `04-domain-model` reserved. A 48-row seed
KPI/Metric Catalogue (42 rows consolidating measurable facts already present in the nine
authored business-domain modules' own data models, 6 new cross-module composite metrics)
activates `14-reporting`'s already-reserved `DashboardWidget.widget_type = METRIC_REFERENCE`/
`metric_ref_id` slot with **zero** additive change to that module's schema — this module
supplies the missing `MetricView` catalogue entity and its own `GET /metric-views/{id}/
reference` resolution endpoint. `DashboardDefinition`/`DashboardWidget` themselves remain
exclusively owned by `REPORTING`; this module introduces no competing dashboard aggregate.
Defines **zero** governed `module_actions` types — an even smaller governance footprint than
`REPORTING`'s own single governed action, since every aggregate here is either ungoverned
catalog content (`MetricDefinition`, `MetricView`) or an append-only measurement fact
(`MetricValue`, mirroring `KRI`/`VendorSLA`'s identical green/amber/red banding shape). Reuses,
never duplicates, `KRI` (`RISK`) and `VendorSLA` (`TPR`) — every KRI-/SLA-derived metric is
computed from that module's own existing bulk endpoint, per `04-domain-model` Assumption 3 and
`25-third-party-risk` Assumption 10. Names one genuinely new PRSMTD capability gap (a generic
formula-execution/BI-computation engine, spec Assumption 7) and one genuinely new ERM content
gap (a roster-of-required-acknowledgers capability needed to compute a true Policy
Acknowledgement Completion rate, surfaced by `MTR-POL-001`). **Two additive changes proposed,
not applied**: an eleventh-bounded-context amendment to `04-domain-model` (spec Assumption 3)
and a `dependencies:`/`DashboardDefinition.audience = REGULATORY` addition to `14-reporting`
(spec Assumption 11) — following the same propose-then-apply-later pattern this repository has
now used many times. No frozen spec was modified this session.

...
- **ERM verification**: every claim about what the ERM specifications provide is drawn from
  the six currently authored, frozen documents — `04-domain-model/01-*`, `10-risk/01-*`,
  `12-controls/01-*`, `11-compliance/01-*`, `13-audit/01-*`, `09-security/01-*` — and their
  Traceability blocks as aggregated in `01-master-traceability-matrix.md`.
  Reserved-but-unauthored bounded contexts (`POLICY`, `INCIDENT`/`ISSUE`/`CAPA`,
...
- This document is a snapshot as of **2026-07-20**, incrementally updated in Session 6 to
  ...
  `09-security/01-security-management.md`, and again in Session 7 to reflect the three
  ...
  `COMPLIANCE`/`AUDIT` status labels — **only the sections those changes affect were
  updated**; all other analysis is preserved unchanged from prior sessions. Both PRSMTD and
  the ERM specification set will keep changing — see `docs/roadmap.md` Risks register for the
  staleness risk this snapshot carries, and re-verify before relying on any rating below at a
  later date.
...
| **Current ERM specification maturity** | Five of ten bounded contexts in the domain model's map are authored, implementation-ready, and internally consistent: `RISK`, `CONTROLS`, `COMPLIANCE`, `AUDIT`, `SECURITY`, plus the cross-cutting `04-domain-model` that ties them together. All five SEBI-grounded core-domain contexts with the most direct near-term regulatory backing are done. **Session 6 authored the sixth authoritative spec**, [`09-security/01-security-management.md`](../09-security/01-security-management.md) (module code `SECURITY`), originally outside the domain model's own nine-context map (a gap that spec's own Assumption 1 named explicitly). **Session 7 closed that gap** — `04-domain-model` now names `SECURITY` as a tenth, authored context, so `SECURITY` is folded into the fraction below rather than sitting outside it (see [Percentage Completion](#percentage-completion)). |
| **Combined platform maturity** | If the five authored GRC specs were implemented today, an AMC would have a genuinely governed risk register, control library, compliance obligation register, internal/system audit capability, and cybersecurity governance capability (security findings/vulnerability management, privileged access grants, secrets/key/certificate governance) — but no policy management, no incident/issue/CAPA, no third-party risk, no business continuity plan register, no reporting/analytics layer, no binary evidence storage, and no SIEM/automated threat-detection capability. See [Percentage Completion](#percentage-completion). |
| **Regulatory readiness** | **Partially ready** for the SEBI Mutual Fund regulatory profile specifically (Risk Management System circular, Master Circular Annexures operational/financial/IT/cyber/compliance-risk/audit sections, and now the Cyber Security and Cyber Resilience Framework's governance layer via `SECURITY`) if the five specs were implemented. **Not ready** for DPDP Act, CERT-In Directions, Companies Act, or Income Tax — none is addressed by any current spec. **Early-stage, architecturally-compatible-only** for ISO 27001/27701/22301/31000, COBIT, and NIST CSF — no ERM spec explicitly cites or maps to any of these; `SECURITY`'s `SecurityPolicyDomain.framework_tag` is designed to carry such a mapping by convention but does not itself constitute a verified crosswalk. |
...
| **Architecture maturity** | High for what exists: every authored spec follows one shared-kernel modeling pattern (`04-domain-model`'s taxonomy shape, governed-lifecycle shape, opaque-reference shape, code-sequence shape), and `13-audit` is the first spec to activate its primary cross-context integrations with **zero** additive changes required to any frozen prior spec — a leading indicator that the shared kernel is working as intended. |
| **Implementation readiness** | Each of the six authored specs is implementation-ready on its own terms (concrete tables, states, APIs, role/permission names) but **none has been implemented in PRSMTD** — this repository is specification-only by design (`CLAUDE.md`). The two small, precisely-scoped additive changes `11-compliance` proposed (`Risk.source = COMPLIANCE_OBLIGATION`; a `CONTROLS`-side obligation-link endpoint) are applied (Session 6). The three further additive changes `09-security/01-*` proposed (`Risk.source = SECURITY_FINDING`; an `AuditEvidence`/`Finding` extension on `13-audit`; a tenth-context row on `04-domain-model`) are now **also applied** (Session 7 — see each target document's own Amendment log). No additive change proposed by any authored spec remains unapplied as of this session. |
| **Highest strength** | The governance-ledger/maker-checker substrate (PRSMTD, Built) plus the immediate-raise/governed-closure exception pattern (ERM, Specified across all four modules) together give every governed GRC decision — risk acceptance, control sign-off, compliance exception closure, audit finding closure — the same real, auditable, SoD-enforced approval mechanism with zero bespoke code per module. |
| **Highest risk** | The platform document/object storage capability gap, first flagged at `12-controls` and now inherited by `COMPLIANCE` and `AUDIT` unchanged: every evidence-bearing control, obligation, and audit finding in this repository's design can be fully governed and metadata-tracked, but **cannot yet retain the actual binary evidence** an auditor or regulator would expect to retrieve, except for `AUDIT`'s new system-trace-extract evidence path (system.md §4.1), which covers only evidence that is itself a platform-trace citation. |
| **Recommended next module** | Session 6 completed both prior Next Milestone items (the two additive changes; `09-security/` consolidation and its `SECURITY` module); Session 7 completed the three additive changes `09-security` had proposed. See `docs/roadmap.md`'s updated Next Milestone for the current recommendation — the persona-to-module-role and `system.md §18` ADRs, scoping `POLICY`, starting `INCIDENT`/`ISSUE`/`CAPA`, or the two still-open capability gaps `09-security/01-*` named (SIEM/security-event correlation; ABAC). |
...
`SECURITY`, `04-domain-model`) were implemented into PRSMTD:
...
**Which roadmap items would still remain before full platform capability exists**: five
reserved bounded contexts (`POLICY`, `INCIDENT`/`ISSUE`/`CAPA`, `THIRD-PARTY RISK`,
...
mechanism (all five additive cross-spec changes proposed across Sessions 4–6 are now applied,
none remain open as of Session 7 — see Roadmap Validation), and — newly surfaced by this
assessment, not previously named on `docs/roadmap.md` — explicit DPDP Act / CERT-In
obligation content and an explicit international-standard crosswalk. See
...
| Policy Management | Not Built | Not Specified (reserved boundary only) | Not Started |
...
| ISO/IEC 27001 | Information security management system | Not Implemented | Not Specified (no explicit citation or crosswalk in any spec) | ❌ Not Yet Specified | Early Stage (architecturally compatible: `CONTROLS`' taxonomy-driven, testable, evidenced control library structurally resembles an ISMS control set) | None (structural resemblance only) | New: author an explicit ISO 27001 Annex A crosswalk once a real implementation exists to map against |
...
| ISO 27001 Annex A (representative: A.5 policies) | Formal, versioned information security policy | No | No (no Policy module) | Yes | `POLICY` (reserved) | — | None yet |
...
| Policy Management | Not Built | Not Specified | Not Started | `POLICY` (reserved) |
...
| ISO/IEC 27001 | Not Started | Early Stage | `CONTROLS`' structure (taxonomy, testable, evidenced) is generically ISMS-shaped but has no explicit Annex A crosswalk. |
...
| Functional | `POLICY`, `INCIDENT`/`ISSUE`/`CAPA`, `THIRD-PARTY RISK` all reserved but unspecified | Each its own future module | `docs/roadmap.md` Next Milestone lists `POLICY` and `INCIDENT`/`ISSUE`/`CAPA` as candidates | Varies per context — see `04-domain-model` Dependency Rules | Medium |
...
| `POLICY` scoping | **Yes** — Next Milestone item 3 |
| `INCIDENT`/`ISSUE`/`CAPA` | **Yes** — Next Milestone item 4 |
...
| `POLICY` | Planned (boundary reserved, no scoping README yet) | Not Started | Named candidate, not current Next Milestone |
| `INCIDENT`/`ISSUE`/`CAPA` | Planned (boundary reserved) | Not Started | Named candidate, not current Next Milestone |
| `THIRD-PARTY RISK` | Planned (boundary reserved) | Not Started | Not currently named as a near-term candidate |
| `BUSINESS CONTINUITY` | Planned (boundary reserved) | Not Started | Named as an alternative candidate historically |
| `REPORTING`/`ANALYTICS` | Planned (Conformist relationship reserved) | Not Started | Not currently named as a near-term candidate |
...
| ERM Specification Maturity | Medium-High, growing | Six documents authored, each meeting `CLAUDE.md`'s full Documentation Standards checklist, each building on a shared modeling kernel that has now held across six consecutive modules without structural rework (`13-audit` needed zero additive changes to any frozen spec at its own authoring; `09-security/01-*` proposed three additive changes, all now applied — Session 7 — the same non-invasive discipline every prior activation used). Five of ten bounded contexts named in `04-domain-model`'s map (now including `SECURITY`, added Session 7) remain unauthored: `POLICY`, `INCIDENT`/`ISSUE`/`CAPA`, `THIRD-PARTY RISK`, `BUSINESS CONTINUITY`, `REPORTING`. |
...
| Traceability Maturity | High | Every substantive spec carries the mandated Traceability block; `01-master-traceability-matrix.md` aggregates all six with zero gaps between what each spec's Traceability block claims and what the matrix records — re-verified this session (Session 7) while correcting the staleness the Phase 2 consistency review found in `04-domain-model` and `10-risk`. |
...
**Numerator**: 5 authored bounded-context module specs (`RISK`, `CONTROLS`, `COMPLIANCE`,
`AUDIT`, `SECURITY`).

**Denominator**: 10 total bounded contexts in `04-domain-model`'s bounded context map (5
authored + 5 reserved: `POLICY`, `INCIDENT`/`ISSUE`/`CAPA`, `THIRD-PARTY RISK`,
`BUSINESS CONTINUITY`, `REPORTING`).

**Specification Completion = 5/10 = 50%.** This figure **changes from Session 6's 4/9 ≈ 44%**
because the denominator itself changed: `04-domain-model` was amended in Session 7 to add
`SECURITY` as a tenth bounded context (closing the gap `09-security/01-*` Assumption 1
discovered), so `SECURITY` now belongs in both the numerator and the denominator rather than
being reported as a fraction-external addendum. This is not a redefinition of what the
fraction measures — it is the direct, mechanical consequence of `04-domain-model`'s own map
changing, the same kind of recount Session 6 already applied to the Platform Capability
Completion figure below when new rows were added.
...
[Platform Capability Matrix](#platform-capability-matrix), updated this session with three
new "Specified" rows (`SECURITY`'s Cybersecurity Governance, Privileged Access Management,
Secrets/Key/Certificate Governance) and two new "Not Started" rows (SIEM/threat detection,
ABAC) — 7 substrate rows + 12 GRC-domain rows + 5 cross-cutting-gap rows. This is an
auditable count, not an effort-weighted estimate — a single "Built" substrate capability
(e.g. RBAC) and a single "Not Started" GRC-domain capability (e.g. Third-Party Risk) are
...
| **Specified but Pending** | 7 | Risk Register, Control Library, Compliance/Regulatory Obligations, Audit Management, Cybersecurity Governance, Privileged Access Management, Secrets/Key/Certificate Governance |
| **Remaining (Not Started/Not Specified)** | 10 | Policy Management, Incident/Issue/CAPA, Third-Party Risk, Business Continuity, Reporting & Analytics, Document/Object Storage, Records Retention Schedule, Notification/Alerting, SIEM/Automated Threat Detection, ABAC |

**Already Built % = 7/24 ≈ 29%. Specified but Pending % = 7/24 ≈ 29%. Remaining % = 10/24 ≈
42%.** (Session 5's 19-row denominator is superseded by this 24-row count; the underlying
substrate rows and their "Built" status are unchanged — only the GRC-domain and gap rows
grew, per `09-security/01-*`.)
...
- **Dependencies**: [`01-master-traceability-matrix.md`](01-master-traceability-matrix.md);
  all six authored specs; `PRSMTD/docs/authoritative/system.md` (re-verified Session 6);
